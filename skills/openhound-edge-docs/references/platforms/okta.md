# Okta Platform Profile

Use this profile for `Okta_` edge docs and OpenHound Okta repositories.

## Quality Bar

`Okta_AddMember.md` is the minimum quality bar: concrete abuse path, concrete cleanup path, runnable Okta API code blocks, verification, opsec, and markdown-link references.

API examples should:

- Start with `OKTA_ORG`, `OKTA_API_TOKEN`, object IDs, and controlled values.
- Use `curl -sS` or `curl -i -sS`.
- Use `Accept: application/json`.
- Use `Authorization: SSWS $OKTA_API_TOKEN` unless OAuth bearer tokens are specifically required. Mention `Authorization: Bearer $OKTA_ACCESS_TOKEN` when OAuth is appropriate.
- Include expected success signals such as `204 No Content`, returned IDs, changed status, missing membership, revoked session, or failed lookup.
